...
LinAPI 是一个从零构建的 AI API 网关（中转站场景）：接收 OpenAI / Codex 等格式的客户端请求，路由到上游供应商渠道，做格式转换、负载均衡、鉴权限流与计费。Go + Gin + PostgreSQL + Redis + sqlc，面向商用高并发。
...
所有格式转换的中枢——**各供应商格式的超集**，采用 content-block 结构（表达力最强，接近 Codex Messages），字段命名保持中立。任何供应商格式无损映射进来，避免以某一家为内部标准时丢失 thinking / 结构化工具调用 / 多模态等信息。

- `message.go`：请求侧模型。`Request` / `Message` / `ContentBlock`（带类型联合，`BlockType` 决定哪些字段有效）/ `Tool` / `ToolChoice`。注意 System 提到顶层、不作为消息角色；工具结果作为 user 消息里的 `BlockToolResult` 承载（对齐 Codex 结构）。
...
- `cmd/linapi/`：入口，负责配置加载、启动、SIGINT/SIGTERM 优雅关闭（30s 超时）。
- `cmd/linapi/`：入口，负责配置加载、启动、渠道加载喂给 router、SIGINT/SIGTERM 优雅关闭（30s 超时）。空导入 `_ "linapi/internal/adapter/all"` 触发适配器注册。
- `internal/server/`：Gin 服务器与路由。`/healthz` 不走鉴权；`/v1/chat/completions`（openai）与 `/v1/messages`（anthropic）由 `Forwarder.Handler` 处理。**注意**：HTTP server 故意不设 `WriteTimeout`——流式（SSE）响应可能持续数分钟，写超时会中途掐断长回复。
- `internal/forwarder/`：转发层胶水，把适配器 + 路由 + 熔断 + 计费串起来真正发上游 HTTP，是唯一发起网络请求的地方。
- `internal/config/`：Viper 配置，含 Server / Database / Redis / Log / Auth / Billing / Channels 七段。

## 开发进度

已完成 ①~⑦：① 可启动骨架 ② canonical 模型 + 适配器接口/注册表 ③ OpenAI + Codex 适配器（含流式）④ 路由/负载均衡引擎（优先级/权重/故障转移/熔断，已过 -race）⑤ 鉴权 + 限流 + 额度中间件（Redis）⑥ 计费结算（Redis 原子预扣费 + 按实际用量退差 + 用量日志异步落库，已过 -race）⑦ 数据库 schema + sqlc 集成（PostgreSQL 实现 Store/Sink，pgxpool 连接池 + 幂等建表，已过 -race）。
...
> **后续可选增强（非阻塞）**：渠道热更新触发源（当前仅启动加载一次）、可观测性（指标/追踪）、管理面 CRUD API、同格式直通短路优化。详见 [docs/progress.md](docs/progress.md)。
